  ...
  if (!req.user) {
    ...
    User.findOne(searchQuery, function (err, user) {
      if (err) {
        ...
        if (!user) {
          user = User.findbyEmail(providerUserProfile.email);
          // Check if user with provider email already exist
          if (!user) {
            user = new User({
            ...
            // User with provider email already exists
            return done(err, user);
          }
        } else {
          // User with provider data already exists
          return done(err, user);
        }
      }
    });
